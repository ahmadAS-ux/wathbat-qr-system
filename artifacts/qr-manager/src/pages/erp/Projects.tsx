...
function CreateProjectModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const [form, setForm] = useState({ name: '', location: '', buildingType: '', productInterest: '', estimatedValue: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name) { setError(t('erp_required_fields')); return; }
    if (!selectedCustomer && (!newName.trim() || !newPhoneE164)) { setError(t('erp_required_fields')); return; }
    await doSubmit();
  };
