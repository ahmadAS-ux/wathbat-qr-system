      ...
      <div className="max-w-4xl mx-auto px-4 py-6" dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <div className={`flex items-center gap-3 mb-6 ${isRtl ? 'flex-row-reverse' : ''}`}>
          <button
            ...
            className="p-2 rounded-xl text-slate-500 hover:text-[#1B2A4A] hover:bg-slate-100 transition-colors"
          >
          ...
          {canEdit && (
            <button
              onClick={openAdd}
              className={`flex items-center gap-2 px-4 py-2 bg-[#1B2A4A] text-white text-sm font-semibold rounded-xl hover:bg-[#142240] transition-colors ${isRtl ? 'font-[Tajawal] flex-row-reverse' : ''}`}
            >
        ...
        <div className={`flex gap-2 mb-4 flex-wrap ${isRtl ? 'flex-row-reverse' : ''}`}>
          <input
            ...
            className={`flex-1 min-w-[180px] px-3 py-2 text-sm border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-[#1B2A4A]/40 ${isRtl ? 'font-[Tajawal] text-end' : ''}`}
          />
          <select
            value={filterCat}
            onChange={e => setFilterCat(e.target.value)}
            className={`px-3 py-2 text-sm border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-[#1B2A4A]/40 ${isRtl ? 'font-[Tajawal] text-end' : ''}`}
          >
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map(v => (
              <div key={v.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                {/* Vendor Row */}
                <div className={`flex items-center gap-3 p-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={`flex items-center gap-2 mt-0.5 flex-wrap ${isRtl ? 'flex-row-reverse' : ''}`}>
                      <span className={`text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200 ${isRtl ? 'font-[Tajawal]' : ''}`}>
                  ...
                  <div className={`flex items-center gap-1 shrink-0 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <button
                      ...
                      className="p-2 rounded-lg text-slate-400 hover:text-[#1B2A4A] hover:bg-slate-100 transition-colors text-xs flex items-center gap-1"
                    >
                      {t('vendor_pos_title')}
                    ...
                    {canEdit && (
                      <button
                        onClick={() => openEdit(v)}
                        className="p-2 rounded-lg text-slate-400 hover:text-[#1B2A4A] hover:bg-slate-100 transition-colors"
                      >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-[#1B2A4A]/40 ${isRtl ? 'font-[Tajawal] text-end' : ''}`}
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className={`w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-[#1B2A4A]/40 bg-white ${isRtl ? 'font-[Tajawal] text-end' : ''}`}
                >
                  {VENDOR_CATEGORIES.map(c => (
                    <option key={c} value={c}>{categoryLabel(c, isRtl, t)}</option>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-[#1B2A4A]/40"
                    dir="ltr"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className={`w-full px-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:border-[#1B2A4A]/40 resize-none ${isRtl ? 'font-[Tajawal] text-end' : ''}`}
                />
              </div>
            </div>
            ...
            <div className={`flex gap-3 justify-end ${isRtl ? 'flex-row-reverse' : ''}`}>
              <button
                ...
                className={`px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-xl transition-colors ${isRtl ? 'font-[Tajawal]' : ''}`}
              >
                {t('erp_cancel')}
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className={`px-4 py-2 text-sm font-semibold bg-[#1B2A4A] text-white rounded-xl hover:bg-[#142240] disabled:opacity-50 transition-colors ${isRtl ? 'font-[Tajawal]' : ''}`}
              >
